...
class UTComp_HudCDeathmatch extends HudCDeathmatch;
...
simulated function DrawTimer(Canvas C)
{
    ...
    else if(BS_xPlayer(PlayerOwner)==None || !BS_xPlayer(PlayerOwner).bInTimedOvertime)
        Seconds=GRI.ElapsedTime-GRI.TimeLimit*60-1;
    else
        Seconds=GRI.ElapsedTime-BS_xPlayer(PlayerOwner).OvertimeEndTime;
